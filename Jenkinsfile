pipeline {
    ...
    stages {
        ...
        stage('Build Docker Image') {
            steps {
                sh 'docker build -t rajeevgangaraju/color-app:${BUILD_NUMBER} .'
            }
        }
        ...
        stage('Trivy Docker Image Scan') {
            steps {
                ...
                sh '''
                    ...
                    --pkg-types os \
                    --severity HIGH,CRITICAL \
                    rajeevgangaraju/color-app:${BUILD_NUMBER}
                '''
            }
        }
        ...
        stage('Push Docker Image') {
            steps {
                withCredentials([usernamePassword(credentialsId: 'docker-hub', passwordVariable: 'PASS', usernameVariable: 'USER')]) {
                    ...
                    sh 'docker push rajeevgangaraju/color-app:${BUILD_NUMBER}'
                }
            }
        }
        ...
        stage('Deploy to Kubernetes') {
            steps {
                ...
                sh "kubectl set image deployment/color-app-deploy color-app=rajeevgangaraju/color-app:${BUILD_NUMBER}"
            }
        }
